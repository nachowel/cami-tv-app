...
test("production Awqat Salah sync keeps Aladhan fallback after repeated Awqat login network resets", async () => {
  const logs: string[] = [];
  let loginAttempts = 0;
  const { db, state } = createFakeDb({
    ...mockDisplayData.prayerTimes,
  ...
  assert.ok(logs.some((message) => /login attempt 3 failed: network reset/i.test(message)));
  assert.ok(logs.some((message) => /Awqat Salah login failed: network reset after 3 attempts/i.test(message)));
  assert.ok(logs.some((message) => /falling back to aladhan/i.test(message)));
