...
export function validatePrayerTimesCurrent(value) {
    const errors = [];
            ...
            for (let i = 0; i < order.length - 1; i++) {
                ...
                if (isValidTime24(current) && isValidTime24(next)) {
                    if (timeToMinutes(current) >= timeToMinutes(next)) {
                        errors.push(`Logical order violation in tomorrow: ${order[i].label} (${current}) is not before ${order[i + 1].label} (${next}).`);
                    }
                }
            }
        }
    ...
    if (today && typeof today === "object") {
        ...
        for (let i = 0; i < order.length - 1; i++) {
            ...
            if (isValidTime24(current) && isValidTime24(next)) {
                if (timeToMinutes(current) >= timeToMinutes(next)) {
                    errors.push(`Logical order violation in today: ${order[i].label} (${current}) is not before ${order[i + 1].label} (${next}).`);
                }
            }
        }
    }
...
export function describePrayerTimesForLog(prayerTimes, timezone, providerSource, docPath) {
    const lines = [];
