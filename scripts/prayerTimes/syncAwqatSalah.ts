...
import { runProductionAwqatSalahSync } from "./syncAwqatSalahRuntime.ts";

function printHelp() {
  console.log("Usage: npm run prayer-times:awqat:sync");
  console.log("");
  console.log("Writes only to prayerTimes/current if source is awqat-salah.");
}
...
export async function syncAwqatSalah(args = process.argv.slice(2), env = process.env) {
...
if (process.argv[1] != null && pathToFileURL(process.argv[1]).href === import.meta.url) {
  void syncAwqatSalah().catch((error) => {
    const message = error instanceof Error ? error.message : "Unexpected Awqat Salah sync error";
    console.error(`Awqat Salah sync command failed: ${message}`);
    process.exitCode = 1;
  });
}
